...
class ItemGenreDescriptionAdapter(private val onClick: (Genre, Int) -> Unit) :
    ...
    class ViewHolder(private val binding: ItemGenreDescriptionBinding ) : RecyclerView.ViewHolder(binding.root) {
        companion object {
            fun from(parent: ViewGroup): ViewHolder {
                val binding =
                    ItemGenreDescriptionBinding.inflate(LayoutInflater.from(parent.context), parent, false)
                return ViewHolder(
                    binding
                )
            }
